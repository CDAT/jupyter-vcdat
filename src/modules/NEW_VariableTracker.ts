...
export default class VariableTracker {
  ...
  get notebookPanel(): NotebookPanel {
    return LabControl.getInstance().notebookPanel;
  }
  ...
  public async getFileVariables(filePath: string): Promise<Variable[]> {
    ...
    try {
      ...
      if (!(await AppControl.getInstance().tryFilePath(path))) {
        this._isBusy = false;
      ...
      const result: string = await LabControl.getInstance().runBackendCode(
