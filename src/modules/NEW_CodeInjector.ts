...
export default class CodeInjector {
  ...
  public async injectImportsCode(index = -1, skip = false): Promise<number> {
    ...
    if (skip) {
      ...
      const output: string = await this.app.labControl.runBackendCode(
        CHECK_MODULES_CMD
      );
    ...
    let cellIdx: number = this.app.labControl.findCellWithMetaKey(
      IMPORT_CELL_KEY
    )[0];
...
canvas = vcs.init(display_target='off')`;
  ...
  public async deleteVariable(variable: Variable): Promise<void> {
    ...
    this.app.varTracker.refreshVariables();
  }
  ...
  public async loadVariable(
    ...
    this.app.varTracker.addVariable(variable);
  }
  ...
  public async loadMultipleVariables(variables: Variable[]): Promise<void> {
    ...
    this.app.varTracker.selectedVariables = newSelection;

    // Refresh the list
    await this.app.varTracker.refreshVariables();
  }
  ...
  public async animate(
    ...
    const selectedVariable: string = this.app.varTracker.findVariableByID(
      this.app.varTracker.selectedVariables[0]
    )[1].alias;
  ...
  public async plot(
    ...
    let selectedVariables: string[] = this.app.varTracker.selectedVariables;
    if (selectedVariables.length > MAX_SLABS) {
      selectedVariables = selectedVariables.slice(0, MAX_SLABS);
      this.app.varTracker.selectedVariables = selectedVariables;
    }
    ...
    let cmd = "";
    const sidecarReady: string = await this.app.labControl.runBackendCode(
      CHECK_SIDECAR_EXISTS_CMD
    );
    ...
    if (previousDisplayMode !== currentDisplayMode) {
      cmd = getSidecarDisplayCommand(
        currentDisplayMode,
        sidecarReady === "True",
        this.app.labControl.notebookPanel.title.label
      );
    }
    ...
    for (const varID of selectedVariables) {
      cmd += `${this.app.varTracker.findVariableByID(varID)[1].alias}, `;
    }
  ...
  private async inject(
    ...
    if (this.app.labControl.notebookPanel === null) {
    ...
    try {
      ...
      const idx: number =
        index ||
        this.app.labControl.notebookPanel.content.model.cells.length - 1;
      const [newIdx, result]: [
        number,
        string
      ...
      this.app.labControl.notebookPanel.content.activeCellIndex = newIdx + 1;
  ...
  private async openCloseFileCmd(
    ...
    const path = Utilities.getUpdatedPath(
      this.app.labControl.notebookPanel.sessionContext.path,
      filePath
    );

    // Check that file can open before adding it as code
    if (await this.app.tryFilePath(path)) {
