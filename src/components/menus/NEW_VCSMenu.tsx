...
> {
  ...
  public async toggleAnimate(): Promise<void> {
    ...
    this._app.labControl.setMetaData(PLOT_OPTIONS_KEY, [
  ...
  public dismissSavePlotSpinnerAlert(): void {
    ...
    this._app.labControl.commands.execute("vcdat:refresh-browser");
  ...
  public saveNotebook(): void {
    ...
    this._app.labControl.setMetaData(
      PLOT_OPTIONS_KEY,
      [
        this.state.overlayMode,
        this.state.currentDisplayMode,
  ...
  public async getCanvasDimensions(): Promise<{
    ...
    try {
      if (this.state.plotReady) {
        ...
        const output: string = await this._app.labControl.runBackendCode(
  ...
  public getPlotOptions(): void {
    ...
    const plotOptions: [
      ...
      DISPLAY_MODE,
      boolean
    ] = this._app.labControl.getMetaData(PLOT_OPTIONS_KEY);
    if (!plotOptions) {
  ...
  public getGraphicsSelections(): void {
    ...
    const gmData: [string, string] = this._app.labControl.getMetaData(
      GRAPHICS_METHOD_KEY
    );
  ...
  public getTemplateSelection(): void {
    ...
    const template: string = this._app.labControl.getMetaData(TEMPLATE_KEY);
  ...
  public async copyGraphicsMethod(
    ...
    await this._app.codeInjector.createCopyOfGM(newName, groupName, methodName);
    ...
    await this._app.labControl.setMetaData(GRAPHICS_METHOD_KEY, [
  ...
  public async updateGraphicsOptions(
    ...
    await this._app.codeInjector.getGraphicMethod(group, name);
  ...
  public async updateColormap(colormapName: string): Promise<void> {
    ...
    await this._app.codeInjector.updateColormapName(
      this.state.selectedGM,
      this.state.selectedGMgroup,
      colormapName
    );
  ...
  public async updateTemplateOptions(templateName: string): Promise<void> {
    // Attempt code injection
    await this._app.codeInjector.getTemplate(templateName);
    // If successful, update the state
    this.setState({
      selectedTemplate: templateName,
    });
    // Save selected graphics method to meta data
    this._app.labControl.setMetaData(TEMPLATE_KEY, templateName);
  }
  ...
  public async plot(): Promise<void> {
    try {
      if (this._app.varTracker.selectedVariables.length === 0) {
        NotebookUtilities.showMessage(
          "Notice",
        ...
        if (this.state.shouldAnimate) {
          ...
          await this._app.codeInjector.animate(
            this.state.selectedGM,
            this.state.selectedGMgroup,
          ...
          // Inject the plot
          await this._app.codeInjector.plot(
            this.state.selectedGM,
            this.state.selectedGMgroup,
            this.state.selectedTemplate,
            this.state.overlayMode,
  ...
  @boundMethod
  public clear(): void {
    this._app.codeInjector.clearPlot();
  }
  ...
  public render(): JSX.Element {
    ...
    const inputModalProps = {
      ...
      inputOptions: this._app.labControl.settings.getSavedPaths(),
      invalidInputMessage:
  ...
  private async handleOptionsChanged(
    ...
    await this._app.labControl.settings.setSavedPaths(options);
  }
}
